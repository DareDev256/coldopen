/**
 * SHORTIIE RAW — "CARRY-ON"  (spatial / RENDERED)
 *
...
/** ids whose upload is a static cover card, from the clip pipeline's motion test */
const POSTER_ONLY = new Set(Object.keys(JSON.parse(fs.readFileSync(path.join(SHARED, 'shots.json'), 'utf8')).posterOnly));
...
/* The wall. Each panel is one of her videos: its poster loads immediately, and
   a short clip is promoted to a video texture only when the camera is near it
   and the frame budget allows. The one upload that is really audio behind a
   cover card carries no video at all — the clip pipeline measured it, nobody
   decided it by eye. */
const ID_BY_TITLE: Record<string, string> = {
  'Drip': 'EmrpNsyVtDQ', 'Strawberry Go-Kart': 'iiYmh9-D_14', "Don't Act New": 'HueUBufXMbs',
  'Way We Move': 'Xedv19NEX-E', 'Angola Kinaxixi Freestyle': 'X8zj9clGQO4', 'Sip Cuca': 'NbJnT5j365M',
  'Fire': '82_xVuYR45c', 'Peace': 'z2BL7wgPsaI', 'Cartier Tints': '9hRUzEGfW7o', 'Sexy No Jutsu': 'WLnquJAMnt0',
};
const panels: Panel[] = units.map(u => {
  const id = ID_BY_TITLE[u.title];
  return {
    title: u.title, sub: u.sub, href: u.href,
    poster: `assets/clips/${id}.jpg`,
    video: POSTER_ONLY.has(id) ? undefined : `assets/clips/${id}.mp4`,
...
const bytes = (p: string) => fs.existsSync(p) ? fs.statSync(p).size : 0;
const mb = (n: number) => (n / 1048576).toFixed(1) + 'MB';
const clipDir = path.join(OUT, 'assets/clips');
const clipTotal = fs.readdirSync(clipDir).filter(f => f.endsWith('.mp4')).reduce((s, f) => s + bytes(path.join(clipDir, f)), 0);
console.log(`\n  first paint carries : montage ${mb(bytes(path.join(OUT, 'assets/montage.mp4')))} + three ${mb(bytes(path.join(OUT, 'js/vendor/three.module.min.js')) + bytes(path.join(OUT, 'js/vendor/three.core.min.js')))}`);
console.log(`  lazy, on promotion  : ${panels.filter(p => p.video).length} clips, ${mb(clipTotal)} total`);
console.log(`  poster-only panels  : ${panels.filter(p => !p.video).map(p => p.title).join(', ') || 'none'}`);
